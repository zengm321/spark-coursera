...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.map(article => article.text.toLowerCase())
      .filter(x => x.split(" ").contains(lang.toLowerCase))
      .count().toInt;
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val langArticlePair = rdd.flatMap(article => {
      val langMentioned = langs.filter(lang => article.text.split(" ").contains(lang))
      langMentioned.map(lang => (lang, article))
    })
    langArticlePair.groupByKey()
  } // hahahah + hehehe
